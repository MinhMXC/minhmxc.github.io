...
import { A_PROJECT } from "../../dt-animations";
...
export default function ProjectsTemplate({ projects, changeScreenBg }: ProjectsTemplateProps) {
  ...
  async function changeModeDescription(project: MpProject, newPercentage: number) {
    ...
    await animate(scope.current, { opacity: 0 }, A_PROJECT.switchModeOpacity.transition);
    setMode(1);
    setProject(project);
    animate(scope.current, { opacity: 1 }, A_PROJECT.switchModeOpacity.transition);
